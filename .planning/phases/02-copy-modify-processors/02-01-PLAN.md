...
must_haves:
  truths:
    ...
    - "Dockerfile has import verification RUN step that fails build if imports broken"
  ...
  <action>
    **Part A: Copy and modify full_pass.py**
    ...
    2. Apply these targeted modifications to /home/drewpullen/clawd/soulprint-rlm/processors/full_pass.py:
       ...
       c. MODIFY delete_user_chunks function (lines 18-44) to read env vars inside function body instead of using module-level vars. Replace references to `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` with `os.getenv("SUPABASE_URL")` and `os.getenv("SUPABASE_SERVICE_KEY")` respectively. This follows ADAPTER-01 decision.
       ...
       g. REMOVE `import httpx` (no longer needed after delete_user_chunks uses adapter pattern or direct calls with inline env vars, and save_chunks_batch is removed). Actually KEEP `import httpx` because delete_user_chunks still uses httpx directly.
