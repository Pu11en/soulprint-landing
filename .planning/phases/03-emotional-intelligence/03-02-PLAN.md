...
**Important:** Use `.select('id', { count: 'exact', head: true })` (NOT `.select('*', ...)`) for efficient count-only query. Selecting 'id' instead of '*' avoids fetching all message data into memory when we only need the count.
...
- The relationship arc query uses `.select('id', { count: 'exact', head: true })` for efficient count-only query (no data returned except count)
  </action>
  <verify>
