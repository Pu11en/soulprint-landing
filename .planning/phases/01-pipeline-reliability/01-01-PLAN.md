...
<task type="auto">
  <name>Task 1: Make chunk saves and fact extraction fail loudly with retry</name>
  <files>rlm-service/processors/full_pass.py, rlm-service/processors/fact_extractor.py</files>
  <action>
...
2. In `run_full_pass_pipeline()`, wrap the chunk save loop (lines 176-184) in a try/except that catches the RuntimeError and adds context:
   - `except RuntimeError as e: raise RuntimeError(f"Chunk save failed at batch {i//batch_size}: {e}") from e`
   - This ensures `trigger_full_pass()` in streaming_import.py catches it and sets `full_pass_status = 'failed'` with the specific error.
...
- `import random` and `import anthropic` present
  ...
  <done>
Chunk saves fail loudly on HTTP errors. Fact extraction retries up to 3 times with exponential backoff. Concurrency reduced from 10 to 5.
  </done>
</task>
...
3. Read `fact_extractor.py` — `extract_facts_parallel` uses concurrency=5 (not 10)
4. Read `fact_extractor.py` — `_extract_with_retry()` implements exponential backoff with jitter
5. Read `memory_generator.py` — `_is_placeholder_memory()` validates content before accepting
6. Read `memory_generator.py` — `generate_memory_section()` retries on placeholder content
</verification>
