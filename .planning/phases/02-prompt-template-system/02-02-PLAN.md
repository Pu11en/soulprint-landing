...
must_haves:
  ...
  key_links:
    ...
    - from: "__tests__/cross-lang/prompt-sync.test.ts"
      to: "rlm-service/prompt_builder.py"
      via: "execSync subprocess calling Python PromptBuilder via temp script"
...
1. **Helper: `callPythonPromptBuilder(version, params) -> string`**
   - Use `execSync` to call Python PromptBuilder via a temp script, following the EXACT pattern established in `__tests__/cross-lang/prompt-hash.test.ts`:
     - Write a temp Python script to `/tmp/test-prompt-sync-{Date.now()}.py`
     - Script does: `import sys; sys.path.insert(0, 'rlm-service'); from prompt_builder import PromptBuilder; ...`
     - Script constructs params from JSON, calls `build_system_prompt()`, prints result with `end=''`
     - Run via `execSync('python3 /tmp/test-prompt-sync-XXX.py', { encoding: 'utf8', cwd: process.cwd() })`
     - Clean up temp file in `finally` block
   - CRITICAL: Verify both the TypeScript import path (`@/lib/soulprint/prompt-builder`) and the Python import path (`sys.path.insert(0, 'rlm-service'); from prompt_builder import PromptBuilder`) are accessible from the test. The `cwd: process.cwd()` ensures rlm-service/ is reachable.
   - The Python call must pass `current_date` and `current_time` keyword arguments to match the fixed values used in the TypeScript test calls.

2. **Test fixture: `testSections`**
   - Realistic section data: soul with personality_traits, communication_style, tone_preferences; identity with archetype; user with name and interests; agents with response_style and behavioral_rules (e.g., `["Always be direct", "Never apologize unnecessarily"]`); tools with capabilities
...
**Test runner:** vitest (already configured). Run with `npx vitest run __tests__/cross-lang/prompt-sync.test.ts`.
  ...
  <verify>
Run `npx vitest run __tests__/cross-lang/prompt-sync.test.ts` -- all tests pass. The v1 and v2 cross-language hash comparisons must pass (this is the core PRMT-03 verification). The Python subprocess calls must succeed (verify no import errors in test output).
  </verify>
  <done>
Cross-language tests verify that TypeScript and Python PromptBuilder produce identical v1 and v2 prompts. The ## REMEMBER block appears after ## CONTEXT in v2 (PRMT-04). Python subprocess wiring works correctly using execSync with temp script pattern from prompt-hash.test.ts. All tests pass.
  </done>
</task>
...
7. Cross-language tests use execSync subprocess pattern (matching __tests__/cross-lang/prompt-hash.test.ts)
</verification>
...
- Cross-language tests use temp Python script + execSync pattern (same as prompt-hash.test.ts)
