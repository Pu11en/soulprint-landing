...
2. Verify the file exports PromptBuilder, PromptVersion, getPromptVersion, PromptParams, and PromptBuilderProfile.
3. Test invalid PROMPT_VERSION fallback (PRMT-02 / Research Pitfall 3): Run `PROMPT_VERSION=invalid node -e "const pb = require('./lib/soulprint/prompt-builder'); const b = new pb.PromptBuilder(); console.log(b.getVersion())"` (or equivalent tsx one-liner). Verify it prints `v1-technical` and produces a console warning about the invalid value. If module format prevents require, use: `PROMPT_VERSION=invalid npx tsx -e "import { PromptBuilder } from './lib/soulprint/prompt-builder'; const b = new PromptBuilder(); console.log(b.getVersion());"` -- must output `v1-technical`.
4. Run `PROMPT_VERSION=invalid npm run build` -- build must succeed (no crash on invalid env var), and the fallback to v1-technical should be logged as a warning during build.
  </verify>
  <done>
PromptBuilder class exists with v1 (exact replica of current behavior) and v2 (natural voice with personality reinforcement after RAG context). Both versions handle all edge cases (null sections, imposter mode, web search). Invalid PROMPT_VERSION values fall back to v1-technical with a console warning (PRMT-02 validated).
  </done>
</task>
...
**Verification approach:** After wiring, the route behavior is identical when PROMPT_VERSION is unset or 'v1-technical' (default). Setting PROMPT_VERSION='v2-natural-voice' in .env.local activates the new style.
  ...
  <verify>
1. Run `npx tsc --noEmit` -- no type errors.
2. Run `npm run build` to confirm the route compiles in the Next.js build.
3. Verify the inline `buildSystemPrompt` function no longer exists in the route file: `grep -c "function buildSystemPrompt" app/api/chat/route.ts` should return 0.
4. Verify PRMT-04 behavioral_rules in ## REMEMBER: Write a quick verification script that instantiates PromptBuilder with v2-natural-voice, passes a test profile containing agents_md with `behavioral_rules: ["Always be direct", "Never apologize unnecessarily"]`, includes a memoryContext string, and captures the output. Then verify:
   - `grep '## REMEMBER' output` finds the section
   - The output after `## REMEMBER` contains the actual behavioral rules text ("Always be direct", "Never apologize unnecessarily")
   - `## CONTEXT` appears BEFORE `## REMEMBER` in the output (indexOf check)
   This can be done via: `npx tsx -e "import { PromptBuilder } from './lib/soulprint/prompt-builder'; const b = new PromptBuilder('v2-natural-voice'); const out = b.buildSystemPrompt({ profile: { soulprint_text: null, import_status: 'complete', ai_name: 'Test', soul_md: null, identity_md: null, user_md: null, agents_md: JSON.stringify({ behavioral_rules: ['Always be direct', 'Never apologize unnecessarily'] }), tools_md: null, memory_md: null }, dailyMemory: null, memoryContext: 'Some retrieved context here', aiName: 'Test', isOwner: true }); console.log(out.includes('Always be direct')); console.log(out.indexOf('## CONTEXT') < out.indexOf('## REMEMBER'));"`
   Both lines must print `true`.
  </verify>
  <done>
Chat route uses PromptBuilder from lib/soulprint/prompt-builder.ts. The inline buildSystemPrompt function is removed. Default behavior (PROMPT_VERSION unset) produces identical v1 prompts. Setting PROMPT_VERSION=v2-natural-voice activates natural voice prompts. Behavioral rules from agents_md appear in the ## REMEMBER section AFTER ## CONTEXT (PRMT-04 verified with actual rule content, not just header existence).
  </done>
</task>
...
7. Invalid PROMPT_VERSION (e.g., `PROMPT_VERSION=invalid`) falls back to v1-technical and logs warning (no crash, no silent failure)
8. v2 prompt with agents_md containing behavioral_rules produces output where those specific rules appear after ## REMEMBER, and ## REMEMBER appears after ## CONTEXT
</verification>
...
- v2 ## REMEMBER block contains actual behavioral_rules content from agents_md (not just the header)
- PROMPT_VERSION env var controls version selection with safe fallback on invalid values
