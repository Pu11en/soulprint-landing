...
1. **Find profiles needing attention** (two queries):
   - **Unscored profiles**: Query `user_profiles` where `quality_breakdown IS NULL AND import_status IN ('quick_ready', 'complete')`, limit 5. These are profiles the import fire-and-forget may have missed.
   - **Low-quality profiles**: Call `supabase.rpc('find_low_quality_profiles', { threshold_score: 60 })`, limit 5.
   - Combined max 10 profiles per run. For unscored profiles, run `calculateQualityBreakdown()` first (initial scoring), then check if refinement is needed. For low-quality profiles, go straight to refinement.
...
IMPORTANT: The refinement should use the EXISTING `/create-soulprint` endpoint on RLM. Do NOT assume a `/refine-section` endpoint exists. The RLM `/create-soulprint` endpoint accepts `{ conversations, user_id, stats? }` and returns `{ soulprint, archetype, soul_md, identity_md, agents_md, user_md, memory_log, conversation_count }` — four markdown files as top-level string fields.

Format the request as:
```typescript
...
Parse the response and extract only the section(s) that need refinement:
```typescript
const rlmResult = await response.json();
// RLM returns soul_md, identity_md, agents_md, user_md as top-level string fields
// Map section types to RLM response field names
const sectionFieldMap: Record<string, string> = {
  soul: 'soul_md',
  identity: 'identity_md',
  user: 'user_md',
  agents: 'agents_md',
  tools: 'tools_md', // Note: RLM may not return tools_md — if missing, skip refinement for tools section
};
const refinedContent = rlmResult[sectionFieldMap[sectionType]];
if (!refinedContent) {
  console.warn(`[QualityRefinement] RLM did not return ${sectionType} section for ${profile.user_id}, skipping`);
  continue;
}
```

NOTE: RLM regenerates ALL sections in a single call. To avoid redundant calls when multiple sections need refinement for the same profile, call `/create-soulprint` ONCE per profile, then extract and update only the low-quality sections from the response.
...
Run `npx tsc --noEmit app/api/cron/quality-refinement/route.ts` -- no type errors. Verify `vercel.json` is valid JSON with both cron entries. Verify the route exports a GET handler. Verify the RLM call uses `RLM_SERVICE_URL` environment variable.
  ...
  <done>
- Cron job finds both unscored profiles (NULL quality_breakdown) AND profiles with any quality metric below 60
- Processes max 10 profiles per run (5 unscored + 5 low-quality, Vercel timeout safety)
- Scores unscored profiles via calculateQualityBreakdown, then checks if refinement needed
- Re-generates low-quality sections via RLM /create-soulprint (one call per profile, extract needed sections)
