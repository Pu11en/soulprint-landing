...
Run tests: `npx vitest run __tests__/unit/prompt-helpers.test.ts` — ALL tests should FAIL (module doesn't exist yet).
  ...
  <verify>Run `npx vitest run __tests__/unit/prompt-helpers.test.ts`; capture exit code. Exit code MUST be non-zero (tests fail because module doesn't exist). If exit code is 0, something is wrong — tests should not pass yet.</verify>
  <done>Test file exists with 13+ test cases covering validation and formatting. All tests fail (non-zero exit code from vitest).</done>
</task>
...
Run tests: `npx vitest run __tests__/unit/prompt-helpers.test.ts` — ALL tests should PASS.
  ...
  <verify>npx vitest run __tests__/unit/prompt-helpers.test.ts — all tests pass (exit code 0)</verify>
