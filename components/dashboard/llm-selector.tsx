...
export function LLMSelector() {
    return (
        <div className="flex flex-col gap-4">
            <h2 className="mb-4 text-lg font-mono text-gray-400">
                ...
                {llmOptions.map((option) => (
                    option.id === "chatgpt" ? (
                        <Link key={option.id} href="/dashboard/chat">
                            <button className="flex h-8 items-center gap-2 rounded-lg bg-[#222222] px-3 py-1.5 transition-colors hover:bg-[#2A2A2A]">
                                <div className="flex h-4 w-4 items-center justify-center rounded-sm bg-[#10A37F]">
                                ...
                                <span className="text-xs font-medium text-[#E5E5E5]">Chat GPT</span>
                            </button>
                        </Link>
                    ) : (
                        <button
                            key={option.id}
                            className={cn(
                                "group flex items-center justify-between rounded-md border border-[#333] bg-[#111] px-6 py-4 text-sm font-mono tracking-wider text-gray-300 transition-all hover:border-gray-500 hover:bg-[#1a1a1a]",
                            )}
                        >
